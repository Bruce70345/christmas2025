...
const ENCRYPTION_KEY = createHash("sha256").update(DATA_ENCRYPTION_SECRET).digest();
...
function encryptValue(value?: string) {
  ...
  const encrypted = Buffer.concat([cipher.update(textEncoder.encode(value)), cipher.final()]);
